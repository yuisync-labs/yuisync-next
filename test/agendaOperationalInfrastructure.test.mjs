...
test('infraestrutura conecta capacidade, transporte e responsible_staff_key', async () => {
  ...
  assert.match(advancedCore, /listAppointmentsCommand/)
  assert.match(advancedCore, /filters: \{ status: 'concluido', start, end \}/)
  ...
  assert.match(advancedCore, /const pendingServices = normalizedAppointments\.filter/)
  assert.match(advancedCore, /const serviceHistory = normalizedAppointments\.filter/)
  assert.doesNotMatch(advancedCore, /calculate_petshop_operational_commissions/)
  assert.match(commissions, /Somente a regra gravada no atendimento entra no fechamento/)
  assert.match(commissions, /não reescrevem o histórico/)
  assert.match(commissions, /regra histórica ausente/)
  assert.match(commissions, /não aplica a configuração atual retroativamente/)
  assert.doesNotMatch(commissions, /Tosa 10%|Banho e demais servicos 5%/)
})
