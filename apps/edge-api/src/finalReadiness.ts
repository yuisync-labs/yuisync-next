...
async function mainSchema(database:D1Database|undefined){
  ...
  try{const row=await database.prepare("SELECT value FROM _yuisync_system_metadata WHERE key='schema_version'").first<{value:string}>();return{status:String(row?.value)==='19'?'ready':'wrong_version',version:row?.value??null}}
