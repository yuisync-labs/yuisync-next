...
import {
  handleCompletedAppointmentCompletionCompat,
  isAppointmentCompletionTarget,
} from './appointmentCompletionCompat'
...
export async function handleCompletedAppointmentCompletionQueryCompat(
  request: Request,
  ...
  return handleCompletedAppointmentCompletionCompat(new Request(request.url.replace('/compat/query', '/compat/rpc'), {
    method: 'POST',
    headers,
    body: JSON.stringify({
