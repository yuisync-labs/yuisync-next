...
export async function executeBillingBooking(env:CompatRuntimeBindings,payload:Record<string,unknown>,context:Context){
 ...
 const statements:D1PreparedStatement[]=[
  ...
  billingAppointmentStatement(env.DB,{tenantId:party.tenantId!,moduleId:party.moduleId!,clientId:party.clientId!,petId:party.petId!,appointmentId:identity.appointmentId,operationKey:identity.operationKey,fingerprint:identity.fingerprint,payload,items,allocations,billingType:intent.type,now}),
 ...
 try{await env.DB.batch(statements)}catch(error){
  ...
  if(existing)return existing.operation_fingerprint===identity.fingerprint?json({data:{appointment_id:existing.appointment_id,idempotent:true,billing_intent:intent.type}}):json({code:'IDEMPOTENCY_KEY_REUSED',appointment_id:existing.appointment_id},409)
 ...
 return json({data:{appointment_id:identity.appointmentId,idempotent:false,billing_intent:intent.type,allocation_count:allocations.length}},201)
}
