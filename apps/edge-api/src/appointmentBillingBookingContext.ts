...
export async function resolveBillingBookingContext(
  request: Request,
  ...
  if ('code' in result && result.code) {
    return { error: Response.json({ code: result.code }, { status: 409 }) }
  }

  const identity = await billingCommandIdentity(request, payload)
  if ('error' in identity) return { error: Response.json({ code: identity.error }, { status: 400 }) }
