...
export async function resolveBillingUpdateContext(request:Request,env:CompatRuntimeBindings,appointmentId:string,payload:Record<string,unknown>){
 ...
 const items=catalog.items||[],desired=await resolveBenefitAllocations({db:env.DB,tenantId:party.tenantId!,moduleId:party.moduleId!,clientId:party.clientId!,serviceItems:items,intent});if(desired.code)return{error:Response.json({code:desired.code},{status:409})}
