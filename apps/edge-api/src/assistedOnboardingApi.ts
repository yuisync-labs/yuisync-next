...
function normalizeStaff(value: unknown): StaffRow[] | null {
  ...
  for (const raw of value) {
    ...
    if (!name || name.length > 160 || !STAFF_KEY.test(key) || seen.has(key)) return null
    seen.add(key)
...
function mergeStaff(existing: StaffRow[], incoming: StaffRow[]): StaffRow[] {
...
async function readSnapshot(database: D1Database, scope: Extract<ResolvedScope, { ok: true }>) {
  ...
  const steps = {
    company: true,
    // No persisted installer/client-admin distinction exists today. Readiness is
    // therefore defined by tenant state, not by whichever admin opened this page.
    administrator: administrators.length > 0,
    team: staff.some((row) => row.active),
...
async function patchOnboarding(
  ...
  const now = Date.now()
  let extensionPatch: Record<string, unknown>

  if (body.step === 'team') {
    if (Object.keys(body).some((key) => !['step', 'staff', 'commission_reset_at'].includes(key))) return json({ code: 'INVALID_ONBOARDING_FIELDS' }, 400)
    const incomingStaff = normalizeStaff(body.staff)
    if (!incomingStaff) return json({ code: 'INVALID_OPERATIONAL_STAFF' }, 400)
    ...
    const extensionRow = await database.prepare(`
      ...
      LIMIT 1
    `).bind(scope.tenantId).first<ExtensionRow>()
    const existingStaff = staffFromExtensions(parseExtensions(extensionRow))
    const staff = mergeStaff(existingStaff, incomingStaff)
    const templatePatch: Record<string, unknown> = { [OPERATIONAL_STAFF_TEMPLATE_KEY]: staff }
    if ('commission_reset_at' in body) {
      ...
      templatePatch[COMMISSION_RESET_TEMPLATE_KEY] = resetAt
    }
    extensionPatch = {
      petshop_operational_staff: staff,
      message_templates: templatePatch,
    }
  } else {
    ...
    extensionPatch = {
      store_business_hours: hours,
      petbot_business_hours: hours,
      petbot_slot_interval_min: String(slotInterval),
      petbot_booking_lead_time_min: String(leadTime),
      petbot_booking_capacity: String(capacity),
    }
  }

  await extensionMergeStatement(database, scope.tenantId, 'petshop', extensionPatch, now).run()
  return json(await readSnapshot(database, scope))
}
