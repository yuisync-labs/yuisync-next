...
// A non-empty canonical table outside an executed/reconciled adapter is blocked
// by migrationReadinessGate instead of being silently treated as migrated.
export const LEGACY_CANONICAL_SOURCE_TABLES = Object.freeze([
  'products','petshop_services','stock_movements','settings','appointments',
  'sales','sale_items','sale_payment_splits','chat_sessions','chat_messages','fiscal_documents',
  'subscription_plans','client_subscriptions','loyalty_settings','loyalty_points',
  'commission_rules','cash_register','invoices','billing_settings','accounting_services',
  'petshop_campaign_logs','petshop_growth_booking_settings','petshop_growth_booking_requests',
  'petshop_growth_leads','petshop_growth_no_show_events','petshop_growth_no_show_policy',
  'petshop_growth_report_cards','support_threads','support_messages','tenant_ai_usage_monthly',
])
...
function ms(value, fallback = null) {
...
function enrichCatalog(base, source, scope) {
  ...
  const legacyByCode = new Map(rows(source, 'petshop_services', scope)
    .map((row) => [text(row.code), row]).filter(([code]) => code))
  return {
    ...base,
    services: base.services.map((service) => {
      ...
      const species = ['dog','cat'].includes(text(legacy.species_target).toLowerCase())
        ? text(legacy.species_target).toLowerCase() : null
      return {
        ...service,
        min_weight_kg: minKg,
        max_weight_kg: maxKg,
...
function legacyBenefits(row) {
  ...
  return arr(parseJson(row.service_items, []))
...
function projectAppointmentsV2(base, source, scope) {
  ...
  const appointments = base.appointments.map((appointment) => {
    ...
    if (machine != null && ![4,7,10].includes(machine)) {
      throw new Error(`LEGACY_GROOMING_MACHINE_INVALID:${appointment.id}:${machine}`)
    }
    ...
    return {
      ...
      subscription_benefit_status: benefits.length ? normalizeBenefitState(legacy) : nullable(legacy.subscription_benefit_status),
...
function projectPackageLedger(source, scope) {
  const subscription_plans = rows(source, 'subscription_plans', scope).map((row) => ({
    tenant_id: scope.tenant_id,
    module_id: scope.module_id,
    id: text(row.id),
    name: text(row.name, 'Pacote'),
    price_cents: cents(row.price),
    billing_cycle: ['monthly','quarterly','annual','custom'].includes(text(row.billing_cycle).toLowerCase())
      ? text(row.billing_cycle).toLowerCase() : 'monthly',
  ...
  for (const appointment of rows(source, 'appointments', scope)) {
    ...
    legacyBenefits(appointment).forEach((benefit, index) => {
      ...
      const benefitKey = nullable(benefit.key || benefit.benefit_key || benefit.service_code || (kind === 'transport' ? 'motodog' : null))
      if (!benefitKey) throw new Error(`LEGACY_PACKAGE_BENEFIT_KEY_MISSING:${appointment.id}:${index}`)
      const state = ['reserved','consumed','released'].includes(text(benefit.status).toLowerCase())
        ? text(benefit.status).toLowerCase() : defaultState
      ...
      subscription_benefit_allocations.push({
        ...
        service_code: kind === 'service' ? nullable(benefit.service_code || benefit.code || benefitKey) : null,
        state,
        operation_key: `legacy-benefit:${appointment.id}:${kind}:${position}:${benefitKey}`,
  ...
  for (const allocation of subscription_benefit_allocations) {
  ...
  return { subscription_plans, client_subscriptions, subscription_benefit_allocations }
}
...
function projectLoyalty(source, scope) {
  ...
  const loyalty_settings = setting ? [{
    ...
    redemption_rate_cents: Math.max(0, integer(setting.redemption_rate_cents,
      setting.redemption_rate ? 100 / Math.max(1, finite(setting.redemption_rate)) : 0)),
    data_json: json({
  ...
  const loyalty_points = rows(source, 'loyalty_points', scope)
    .sort((left, right) => (ms(left.created_at, 0) ?? 0) - (ms(right.created_at, 0) ?? 0) || text(left.id).localeCompare(text(right.id), 'en'))
    .map((row) => {
...
function projectCommercialAdmin(source, scope) {
  const commission_rules = rows(source, 'commission_rules', scope).map((row) => {
    ...
    return {
      ...
      module_id: scope.module_id,
      id: text(row.id),
      staff_key: nullable(row.profile_id || row.staff_key),
      service_code: nullable(row.service_code),
      rule_type: type,
      ...
      data_json: json({ applies_to: nullable(row.applies_to) }),
      updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
    }
  }).filter((row) => row.id)

  const cash_register = rows(source, 'cash_register', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),
    opened_by: nullable(row.opened_by),closed_by: nullable(row.closed_by),
    opening_balance_cents: cents(row.opening_balance),
    closing_balance_cents: row.closing_balance == null ? null : cents(row.closing_balance),
    expected_balance_cents: row.expected_balance == null ? null : cents(row.expected_balance),
    difference_cents: row.difference == null ? null : Math.round(finite(row.difference) * 100),
  ...
  const invoices = rows(source, 'invoices', scope).map((row) => ({
    ...
    status: ['pending','paid','cancelled','overdue','refunded'].includes(text(row.status).toLowerCase()) ? text(row.status).toLowerCase() : 'pending',
    due_date: nullable(row.due_date),paid_at_ms: ms(row.paid_at, null),customer_phone: nullable(row.customer_phone),notes: nullable(row.notes),
    invoice_nfe_url: nullable(row.invoice_nfe_url),created_at_ms: ms(row.created_at, 0) ?? 0,updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
  })).filter((row) => row.id)

  const billing = singleton(source, 'billing_settings', scope)
  const billing_settings = billing ? [{
    tenant_id: scope.tenant_id,module_id: scope.module_id,
    data_json: canonicalJson(billing, {}),updated_at_ms: ms(billing.updated_at, ms(billing.created_at, 0)) ?? 0,
  }] : []

  const accounting_services = rows(source, 'accounting_services', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),name: text(row.name, 'Servico'),
    amount_cents: cents(row.amount ?? row.price),status: row.active === false ? 'inactive' : 'active',
    data_json: canonicalJson(row.details ?? row.metadata, {}),created_at_ms: ms(row.created_at, 0) ?? 0,updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
  })).filter((row) => row.id)
...
function projectGrowth(source, scope) {
  ...
  const petshop_growth_booking_settings = setting ? [{
    tenant_id: scope.tenant_id,module_id: scope.module_id,enabled: bool(setting.enabled, true) ? 1 : 0,
    public_slug: text(setting.public_slug),allow_whatsapp_fallback: bool(setting.allow_whatsapp_fallback, true) ? 1 : 0,
    lead_expiration_hours: Math.max(1, integer(setting.lead_expiration_hours, 6)),intake_message: nullable(setting.intake_message),
    updated_at_ms: ms(setting.updated_at, ms(setting.created_at, 0)) ?? 0,
  }] : []

  const petshop_growth_leads = rows(source, 'petshop_growth_leads', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),client_id: nullable(row.client_id),
    source: text(row.source, 'manual'),stage: text(row.stage, 'new'),
    priority: ['low','normal','high','urgent'].includes(text(row.priority).toLowerCase()) ? text(row.priority).toLowerCase() : 'normal',
    ...
    next_followup_at_ms: ms(row.next_followup_at, null),last_contact_at_ms: ms(row.last_contact_at, null),converted_sale_id: nullable(row.converted_sale_id),
  ...
  const petshop_growth_booking_requests = rows(source, 'petshop_growth_booking_requests', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),client_id: nullable(row.client_id),lead_id: nullable(row.lead_id),
    channel: text(row.channel, 'manual'),customer_name: text(row.customer_name, 'Cliente'),pet_name: nullable(row.pet_name),phone: nullable(row.phone),
    service_interest: nullable(row.service_interest),preferred_date: nullable(row.preferred_date),preferred_period: nullable(row.preferred_period),
    transport_mode: text(row.transport_mode, 'dropoff'),need_motodog: bool(row.need_motodog, false) ? 1 : 0,
    motodog_fee_cents: cents(row.motodog_fee ?? row.motodog_fee_amount),pickup_address: nullable(row.pickup_address),
    pickup_neighborhood: nullable(row.pickup_neighborhood),pickup_city: nullable(row.pickup_city),status: text(row.status, 'pending'),notes: nullable(row.notes),
    created_at_ms: ms(row.created_at, 0) ?? 0,updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
  })).filter((row) => row.id)

  const policy = singleton(source, 'petshop_growth_no_show_policy', scope)
  const petshop_growth_no_show_policy = policy ? [{
    tenant_id: scope.tenant_id,module_id: scope.module_id,require_prepayment: bool(policy.require_prepayment, false) ? 1 : 0,
    prepayment_cents: cents(policy.prepayment_amount ?? policy.prepayment),grace_minutes: Math.max(0, integer(policy.grace_minutes, 15)),
    max_strikes: Math.max(1, integer(policy.max_strikes, 2)),auto_block_days: Math.max(0, integer(policy.auto_block_days, 30)),
    reminder_minutes_before: Math.max(0, integer(policy.reminder_minutes_before, 90)),updated_at_ms: ms(policy.updated_at, ms(policy.created_at, 0)) ?? 0,
  }] : []

  const petshop_growth_no_show_events = rows(source, 'petshop_growth_no_show_events', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),appointment_id: nullable(row.appointment_id),client_id: nullable(row.client_id),
    event_type: text(row.event_type, 'no_show'),fee_cents: cents(row.fee_amount ?? row.fee),notes: nullable(row.notes || row.reason),
    created_at_ms: ms(row.created_at, 0) ?? 0,
  })).filter((row) => row.id)

  const petshop_campaign_logs = rows(source, 'petshop_campaign_logs', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),campaign_type: nullable(row.campaign_type),client_id: nullable(row.client_id),
    channel: nullable(row.channel),status: nullable(row.status),payload_json: json(safePayload({
      audience_name: row.audience_name,message: row.message,sent_at: row.sent_at,payload: parseJson(row.payload, null),
    })),created_at_ms: ms(row.created_at, 0) ?? 0,
  })).filter((row) => row.id)

  const petshop_growth_report_cards = rows(source, 'petshop_growth_report_cards', scope).map((row) => ({
    tenant_id: scope.tenant_id,module_id: scope.module_id,id: text(row.id),
    period_key: text(row.period_key, nullable(row.appointment_id) || new Date(ms(row.created_at, 0) ?? 0).toISOString().slice(0,10)),
    metrics_json: json(safePayload({
      appointment_id: row.appointment_id,client_id: row.client_id,pet_name: row.pet_name,summary: row.summary,care_tips: row.care_tips,
      recommended_services: parseJson(row.recommended_services, []),next_visit_date: row.next_visit_date,
      delivery_channel: row.delivery_channel,delivered: row.delivered,created_by: row.created_by,
    })),created_at_ms: ms(row.created_at, 0) ?? 0,updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
  })).filter((row) => row.id)

  return {
    petshop_growth_booking_settings,petshop_growth_leads,petshop_growth_booking_requests,
    petshop_growth_no_show_policy,petshop_growth_no_show_events,petshop_campaign_logs,petshop_growth_report_cards,
  }
}
...
function projectSupport(source, scope) {
  const support_threads = rows(source, 'support_threads', scope).map((row) => ({
    ...
    assigned_to: nullable(row.assigned_to),
    status: text(row.status).toLowerCase() === 'finalized' ? 'closed'
      : (['open','pending','closed'].includes(text(row.status).toLowerCase()) ? text(row.status).toLowerCase() : 'pending'),
    priority: ['low','normal','high','urgent'].includes(text(row.priority).toLowerCase()) ? text(row.priority).toLowerCase() : 'normal',
    source: text(row.source, 'widget'),subject: nullable(row.subject),last_message_preview: nullable(row.last_message_preview),
  ...
  const support_messages = rows(source, 'support_messages', scope).map((row) => ({
    ...
    sender_type: ['customer','agent','system'].includes(text(row.sender_type).toLowerCase()) ? text(row.sender_type).toLowerCase() : 'system',
    body: text(row.body),created_at_ms: ms(row.created_at, 0) ?? 0,
  })).filter((row) => row.id && row.thread_id)
  return { support_threads, support_messages }
}

function projectAiUsage(source, scope) {
  return {
    tenant_ai_usage_monthly: rows(source, 'tenant_ai_usage_monthly', scope).map((row) => ({
      tenant_id: scope.tenant_id,module_id: scope.module_id,
      month_key: text(row.month_key || row.period_month),
      request_count: Math.max(0, integer(row.request_count, 0)),
      token_count: Math.max(0, integer(row.token_count ?? row.tokens_used, 0)),
      updated_at_ms: ms(row.updated_at, ms(row.created_at, 0)) ?? 0,
